...
| R3-F01…F06 | Yönetim paneli: kullanıcı ekle / aç-kapat, `is_admin` + kilitlenme koruması, scope ver/al, roller (yalnız admin), scope kaynağı, boş durumlar | d4cffbc | bekliyor — önce panel, sonra ekler (kullanıcı kararı) | | |
| R3-F07 | İlk yönetici: `~/nix`'te agenix sırrı, Rust açılışta dosyadaki e-postaları aktif admin yapar; listeden çıkarmak geri almaz; CLI yok (KNOW-320) | — | bekliyor | | |
