...
| G2 | 403'ler `security_events`'e yazılmıyor (Python: tek merkezden exception handler, `bda6531`). Şema v2'de `permission_denied` türü hazır bekliyor. | R1 çıkarımı | R1 uygulaması |
...
|---|---|---|---|---|---|
| R1-F01 | PWA kurulumu: `manifest.json` (standalone, ikonlar) + iOS meta. Service worker push'la birlikte (Faz 3). | 552700b, 8fceb4d | bekliyor | | |
| R1-F02 | 403 → `security_events` `permission_denied` (G2) | bda6531 | bekliyor | | |
| R1-F08 | Kayıt başlığında "Açan: X · tarih" | 5cb100b | bekliyor | | |
