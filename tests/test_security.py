...
from shared import db, identity, seed  # noqa: E402


@pytest.fixture(scope="module")
def client():
    from conftest import setup_database  # noqa: E402
    setup_database("security")
...
def _subprocess(env_extra: dict) -> subprocess.CompletedProcess:
...
def test_yayinda_sahte_kimlik_acilmaz():
    r = _subprocess({"EKIPTAKIP_ENV": "yayin", "EKIPTAKIP_AUTH": "sahte",
...
def test_gelistirmede_sahte_kimlik_acilir():
    r = _subprocess({"EKIPTAKIP_AUTH": "sahte", "EKIPTAKIP_ENV": "gelistirme",
                     "EKIPTAKIP_SECRET_KEY": "k" * 40})
...
def test_sahte_kimlik_acik_bayrak_olmadan_acilmaz():
    ...
    """
    r = _subprocess({"EKIPTAKIP_AUTH": "sahte", "EKIPTAKIP_SECRET_KEY": "k" * 40})
    assert r.returncode != 0
...
def test_kurcalanan_oturum_cerezi_gecersiz(client):
    ...
    name = next(k for k in client.cookies.keys() if "ekiptakip" in k)
...
def test_davetsiz_eposta_giremez_ve_kullanici_olusmaz():
    before = db.q1("select count(*) c from users")["c"]
    user, reason = identity.can_enter("yabanci@ornek.com", "google-sub-yok")
    assert user is None and reason == "uninvited"
    assert db.q1("select count(*) c from users")["c"] == before
...
def test_eposta_buyuk_kucuk_harf_ayirmiyor():
    ...
    found, reason = identity.can_enter(u["email"].upper(), "yeni-sub-1")
    assert reason is None and found["id"] == u["id"]
...
def test_ayni_eposta_farkli_google_hesabi_reddedilir():
    ...
    found, reason = identity.can_enter(u["email"], "baska-sub")
    assert found is None and reason == "account_mismatch"
...
def test_pasif_kullanici_giris_yapamaz():
    u = db.q1("select * from users where name = 'Deniz'")
    db.x("update users set is_active = false where id = %s", (u["id"],))
    found, reason = identity.can_enter(u["email"], "sub-x")
    assert found is None and reason == "inactive"
    db.x("update users set is_active = true where id = %s", (u["id"],))


def test_donus_adresi_yalnizca_kendi_yolumuz():
    """AC-7: acik yonlendirme kapali."""
    assert identity.safe_return("/tasks") == "/tasks"
    for bad in ("https://baska.site/x", "//baska.site", "http://x", None, ""):
        assert identity.safe_return(bad) == "/"
...
def test_okuma_istekleri_token_istemez(client):
    assert client.get("/tasks", headers={"X-CSRF-Token": ""}).status_code == 200
...
def test_csrf_reddi_denetim_izine_yazilir(client):
    ...
    csrf_attach(client)
    before = db.q1("select count(*) c from security_events where event_type='permission_denied'")["c"]
    it = db.q1("select id from items limit 1")
    client.post(f"/item/{it['id']}/message", data={"body": "x"},
                headers={"X-CSRF-Token": "yanlis"})
    after = db.q1("select count(*) c from security_events where event_type='permission_denied'")["c"]
...
def test_kimliksiz_csrf_reddi_denetime_yazilmaz(client):
    ...
    before = db.q1("select count(*) c from security_events")["c"]
    for _ in range(5):
        client.post("/logout", headers={"X-CSRF-Token": "yanlis"})
    assert db.q1("select count(*) c from security_events")["c"] == before
    csrf_attach(client)
...
def test_gocler_idempotent_ve_kayitli():
    ...
    """
    assert db.migrate() == []                       # fixture zaten uygulamisti
    applied = {r["name"] for r in db.q("select name from schema_migrations")}
...
def test_arama_sutunu_kendini_gunceller():
    ...
    found = db.q("select title from items where search_vector @@ to_tsquery('tr', %s)", ("vinc:*",))
    assert any("vinç halatı" in r["title"] for r in found)
...
def test_sablonlarda_satir_ici_script_yok():
    ...
    for path in Path(ROOT).rglob("sites/*/templates/**/*.html"):
        text = path.read_text(encoding="utf-8")
        assert "<script>" not in text, path
        assert "hx-on" not in text, path          # htmx onu new Function ile derler


def test_giris_hiz_siniri(client):
    """AC-10: kaba kuvvete karsi dakikada 10 deneme."""
    from shared import hardening
    hardening._history.clear()
    codes = [client.get("/login", follow_redirects=False).status_code for _ in range(12)]
    assert 429 in codes
    assert codes.index(429) >= hardening.LIMIT
    hardening._history.clear()
...
def test_403_denetim_izine_yazilir(client):
    ...
    before = db.q1("select count(*) c from security_events where event_type=%s", ("permission_denied",))["c"]
    r = client.post(f"/item/{it['id']}/message", data={"body": "kapsam disi"})
    assert r.status_code == 403
    assert db.q1("select count(*) c from security_events where event_type=%s", ("permission_denied",))["c"] == before + 1
